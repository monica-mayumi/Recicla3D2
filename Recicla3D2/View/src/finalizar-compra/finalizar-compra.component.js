...
const html2pdf = require("../../node_modules/html2pdf.js/dist/html2pdf");
require("./finalizar-compra.component.css");
require("bootstrap");
class FinalizarCompra {
        ...
        $("input").each((_, e) => {
            ...
            if (e.id === 'cpf') {
                ...
                e.addEventListener('input', function () {
                    ...
                    try {
                        $.get(`https://viacep.com.br/ws/${this.value}/json/`, datas => {
                            if (!datas.erro) {
                                $("#bairro")[0].value = datas.bairro;
                                $("#cidade")[0].value = datas.localidade;
                                $("#rua")[0].value = datas.logradouro;
                                $("#estado")[0].value = datas.uf;
                            }
                        });
                    }
                    catch (e) {
    ...
    verificarCampos() {
        ...
        const validate = (/^\d{3}\.\d{3}\.\d{3}\-\d{2}$/).test($("#cpf")[0].value) && (/^\d{5}-\d{3}/).test($("#cep")[0].value) && (/^\d{2} \d{5}-\d{4}/).test($("#fone")[0].value);
        const emptyInput = (Object)['values'](this.pedido).includes('') ? false : true;
        if (validate && emptyInput)
            $("#baixar-boleto").removeAttr("disabled");
        else
            $("#baixar-boleto").attr("disabled", "disabled");
        $("#baixar-boleto").click(() => $("#finalizar-compra").removeAttr("disabled"));
    }
    sendRequest(objectToRequest) {
        JSON.parse(localStorage.getItem('carrinho')).idProduct.forEach((e, i) => {
            const pedido = {
                ...
                cep: $("#cep")[0].value,
                cpf: $("#cpf")[0].value,
                state: $("#estado")[0].value,
                city: $("#cidade")[0].value,
                district: $("#bairro")[0].value,
                street: $("#rua")[0].value,
                number: Number($("#numero")[0].value),
                phone: $("#fone")[0].value
            };
            $.ajax({
                type: "POST",
    ...
    downloadPDF() {
        const dateCurrent = new Date().toJSON().slice(0, 10).replace(/-/g, '/').split('/');
        const totalPreco = JSON.parse(localStorage.getItem('carrinho')).prodPreco.reduce((acumulator, preco) => acumulator + preco);
        $("#container").html(`
...
const finalizarCompra = new FinalizarCompra();
